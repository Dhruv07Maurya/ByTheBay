...
export const DitherShader: React.FC<DitherShaderProps> = ({
  ...
  const applyDithering = useCallback(
    (
      ...
      // Clear with background if not transparent
      ...
      const ditheredData = ctx.createImageData(pixelWidth, pixelHeight);
      const out = ditheredData.data;
      ...
      for (let py = 0; py < pixelHeight; py++) {
        for (let px = 0; px < pixelWidth; px++) {
          // Map small canvas coordinates to source image coordinates
          const srcX = Math.floor((px / pixelWidth) * sourceWidth);
          const srcY = Math.floor((py / pixelHeight) * sourceHeight);
          ...
          const outIdx = (py * pixelWidth + px) * 4;
          ...
          const matrixX = px % matrixSize;
          const matrixY = py % matrixSize;
          ...
          switch (ditherMode) {
            ...
            case "noise": {
              const noiseVal =
                Math.sin(px * 12.9898 + py * 78.233 + time * 100) * 43758.5453;
              ditherThreshold = noiseVal - Math.floor(noiseVal);
              break;
            }
            case "crosshatch": {
              const line1 = (px + py) % 2 < 1 ? 1 : 0;
              const line2 =
                (px - py + 4) % 2 < 1 ? 1 : 0;
          ...
          out[outIdx] = outputColor[0];
          out[outIdx + 1] = outputColor[1];
          out[outIdx + 2] = outputColor[2];
          out[outIdx + 3] = 255;
        }
      }
  ...
  useEffect(() => {
    ...
    const processImage = (img: HTMLImageElement) => {
      ...
      const pixelWidth = Math.ceil(displayWidth / effectivePixelSize);
      const pixelHeight = Math.ceil(displayHeight / effectivePixelSize);
      ...
      // Create offscreen canvas to get image data at pixel scale
      ...
      if (objectFit === "cover") {
        const scale = Math.max(pixelWidth / iw, pixelHeight / ih);
        dw = Math.ceil(iw * scale);
        dh = Math.ceil(ih * scale);
        dx = Math.floor((pixelWidth - dw) / 2);
        dy = Math.floor((pixelHeight - dh) / 2);
      } else if (objectFit === "contain") {
        const scale = Math.min(pixelWidth / iw, pixelHeight / ih);
        dw = Math.ceil(iw * scale);
        dh = Math.ceil(ih * scale);
        dx = Math.floor((pixelWidth - dw) / 2);
        dy = Math.floor((pixelHeight - dh) / 2);
      ...
      // Initial render
      applyDithering(ctx, pixelWidth, pixelHeight, 0);
      ...
      if (animated) {
        const animate = () => {
          ...
          applyDithering(ctx, pixelWidth, pixelHeight, timeRef.current);
